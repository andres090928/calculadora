...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleClick(event) {
        ...
        } else if (valorBoton === '%') { // Nueva condición para calcular el porcentaje
            display.value = calcularPorcentaje(display.value);
        } else {
    ...
    function calcularPorcentaje(expresion) {
        let resultado = evaluarExpresion(expresion); // Primero evaluamos la expresión
        resultado /= 100; // Calculamos el porcentaje
        return resultado;
    }
